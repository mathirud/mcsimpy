...
def J(eta):
    ...
    return np.block([[Rzyx(eta), np.zeros((3, 3))], [np.zeros((3, 3)), Tzyx(eta)]])
...
def read_tf(file_path, tf_type="motion"):
    ...
    T = J([0.0, 0.0, 0.0, 0.0, np.pi, 0.0]) @ np.ones(6)
...
def read_hydrod(filepath):
    ...
    axis system to the mcsimpy axis system.
    ...
    H = np.block([[np.eye(3), Smat(r_g).T], [np.zeros((3, 3)), np.eye(3)]])
...
def read_wave_drift(filepath):
    ...
    is transformed from the Veres axis system to the mcsimpy axis system. The
    wave drift data is transformed to the mcsimpy axis system by applying a
    ...
    drift_frc_n : array_like
        ...
        novel). The array is transformed to the mcsimpy axis system.
    """
...
def plot_raos(
    ...
    if plot_polar:
        ...
        for i in range(6):
            ...
            plt.plot(
                np.angle(raos[i, :, wave_angle, 0]), np.abs(raos[i, :, wave_angle, 0])
            )
            plt.plot(
...
def _complete_sector_coeffs(vessel_config: dict):
    ...
    forceRAOc_new = np.zeros(
        (6, freqs.size, heading_new.size, vel.size), dtype=np.complex128
    )
...
def generate_config_file(input_files_paths: list = None, input_file_dir: str = None):
    ...
    if input_files_paths is not None:
        ...
        if not sorted(input_file_types) == sorted(file_type_requirm):
            raise ValueError(
                f"The correct files are not distributed. Want: {file_type_requirm} files."
            )
        else:
        ...
        input_file_types = [
            ...
            if "re" in file.split(".")[-1]
        ]
        print(
            sorted(
                [
                    os.path.join(input_file_dir, file)
                    for file in os.listdir(input_file_dir)
                    if "re" in file.split(".")[-1]
                ]
            )
        )
        re1, re2, re7, re8, *_ = sorted(
            [
                os.path.join(input_file_dir, file)
                for file in os.listdir(input_file_dir)
                if "re" in file.split(".")[-1]
    ...
    if headings.size <= 19:
        print(
            f"VERES results only for heading {np.min(headings)} to {np.max(headings)}. "
        )
    ...
    if method == 2:
        ...
        for i in range(maxiter):
            result = least_squares(
                fun_residuals, result.x, args=(w, h, weights), loss="soft_l1"
            )
...
def _invfreqs_alt(g, worN, nB, nA, wf=None, nk=0):
    ...
    mM = np.mat(
        np.hstack(
            (mH * np.mat(mD[:, -nA:]), -np.mat(mD[:, -nk - nB - 1 :][:, : nB + 1]))
        )
    )
    mW = np.mat(np.diag(wf))
    Y = np.linalg.solve(
        np.real(mM.H * mW * mM), -np.real(mM.H * mW * mH * np.mat(mD)[:, -nA - 1])
    )
    a = np.ones(nA + 1)
    a[1:] = Y[:nA].flatten()
    b = np.zeros(nB + nk + 1)
...
def system_identification(w, A, B, max_order=10, method=0, plot_estimate=False):
    ...
    treshold = 0.99
    dofs = np.array(
        [[0, 0], [1, 1], [1, 3], [1, 5], [2, 2], [2, 4], [3, 3], [3, 5], [4, 4], [5, 5]]
    )
        ...
        while not sucess:
            try:
                if np.sum(a) == 0.0:
                    print("a is zero")
                    break
                a_inf_ik, a_est_ik, b_est_ik, kw_est_ik = joint_identification(
                    w, a, b, order, method=method, plot_estimate=False
                )
                ...
                if (rsqrb >= treshold) and (rsqra >= treshold):
                    sucess = True
                    print(
                        f"Joint identification successful for DOF {dof_ik+1}. Order = {order}."
                    )
                    ...
                    if plot_estimate:
                        ...
                        plt.plot(
                            w, np.abs(b + 1j * w * (a - a_inf_ik)), "o", label="|Kw|"
                        )
                        plt.legend()
                        plt.show()
                        ...
                        fig.suptitle(
                            f"Joint identification of DOF {dof_ik+1} with order {order}"
                        )
                        plt.sca(ax[0])
                        plt.title(
                            "Estimated added mass.   rsqra = {:.3f}".format(rsqra)
                        )
                        plt.plot(w, a, "o", label="A")
                ...
                if order > max_order:
                    print(
                        "Could not find a good fit for order less than max_order = {}.".format(
                            max_order
                        )
                    )
                    ...
                    a_inf_ik, a_est_ik, b_est_ik, kw_est_ik = joint_identification(
                        w, a, b, order, method=method, plot_estimate=plot_estimate
                    )
                    # Compute some values to check if the fit is good.
                    sseb = np.sum((b - b_est_ik) ** 2)
                    sstb = np.sum((b - np.mean(b)) ** 2)
                    ...
                    plt.show()
                    yn = input(
                        "If the fit is good, enter 'y' to continue. Otherwise, enter 'n' to run a different order. "
                    )
                    if yn.lower() == "y":
                        sucess = True
                        print(
                ...
                print(
                    f"Joint identification failed for {dof_ik}. Using order {order+1} instead."
                )
                order += 1
                if order > max_order:
...
def quat2eul(w, x, y, z):
    ...
    S = np.array([[0, -eps[2], eps[1]], [eps[2], 0, -eps[0]], [-eps[1], eps[0], 0]])
