...
class EKF:
    ...
    def __init__(self, dt, M, D, Tp, x0=np.zeros(15), P0=np.zeros((15, 15))):
        ...
        self._Rd = np.array([[100, 0, 0], [0, 100, 0], [0, 0, 500 * np.pi / 180]])
    ...
    def predictor(self, tau):
        ...
        self._Pbar = phi @ self._Phat @ (phi.T) + self._gamma @ self._Qd @ (
            self._gamma.T
        )
    ...
    def corrector(self, y):
        ...
        if (
            np.any(np.isnan(y)) == True
        ):  # If no new measurements: Set corrector equal to predictor (Dead reckoning)
            ...
            prediction_error[2] = pipi(
                prediction_error[2]
            )  # Smallest signed angle modification

            self._Phat = parenthesis @ self._Pbar @ (parenthesis.T) + K @ self._Rd @ (
                K.T
            )
    ...
    def state_function_jacobian(self):  # df/dx
        ...
        row2 = np.concatenate(
            (np.zeros((3, 6)), del_f2, np.zeros((3, 3)), Rz(psi)), axis=1
        )
        row3 = np.zeros((3, 15))
        row4 = np.concatenate(
            (np.zeros((3, 6)), del_f4, self._Minv @ (Rz(psi).T), -self._Minv @ self._D),
            axis=1,
        )
