...
class LTVKF:
    ...
    def __init__(self, dt, M, D, Tp, x0=np.zeros(15), P0=np.zeros((15, 15))):
        ...
        ), np.array([[100, 0, 0], [0, 100, 0], [0, 0, 500 * np.pi / 180]])
        ...
        self._Ed[3:6, 0:3], self._Ed[9:12, 3:6] = (
            np.diag([1, 1, 1]) * self._dt,
            np.eye(3) * self._dt,
        )  # Multipliy with .01
    ...
    def corrector(self, y, dead_reckoning):
        if (
            dead_reckoning
        ):  # If dead reckoning: Set corrector equal to predictor (Dead reckoning)
            self.Phat = self.Pbar
            self.xhat = self.xbar
        else:
            K = self.KF_gain
