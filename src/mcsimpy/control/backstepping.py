...
class BacksteppingController:
    ...
    See Also
    --------
    mcsimpy.guidance.path_parm.WayPointPathParam :
    ...
    def virtual_control_dot(
        ...
        line1 = (
            self.K1 @ Smat(nu[2]) @ z1
            - self.K1 @ nu
            - Smat(nu[2]) @ Rz(eta[2]).T @ eta_d_s * u_s
        )
        line2 = Rz(eta[2]).T @ eta_d_s * ddt_u_s
        line3 = (
            self.K1 @ Rz(eta[2]).T @ eta_d_s
            + Rz(eta[2]).T @ eta_d_s2 * u_s
            + Rz(eta[2]).T @ eta_d_s * u_s_dot
        ) * s_dot
